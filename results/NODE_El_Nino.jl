...
plot_lyapunov_exp(sol_node, 15:5:30, 5:5:20, k_values = 0:10:700)
...
best_temp_model, best_temp_neural_ode = load_NODE("temp_model.jld2","temp_neural_ode.jld2")
...
for i in 1:n_restarts
    ...
    sol_ode = solve(prob, Tsit5(), saveat=tstart:tend)
    #plot_SINDy_and_ElNino_trajectories(savePath, X_vector, sol_sindy)
    MSE += loss(reduce(hcat,sol_ode.u)[1,:], x_temp[i:i+tend]) #properly align delay solution against real data
end
